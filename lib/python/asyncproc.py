...
def child_pids(parent_pid):
    return map( int, system('ps -o pid h --ppid %d' % parent_pid).split() )
...
class Process(object):
    ...
    def __del__(self, __killer=os.kill, __sigkill=signal.SIGKILL):
        ...
        #self.__stdout_thread.kill()
        #self.__stderr_thread.kill()
    ...
    def terminate(self, graceperiod=1):
        ...
        pids_to_kill = tree_pids(self.pid())
        ...
        for pid in tree_pids(self.pid()): # any survives?
            os.kill(pid, signal.SIGKILL)
        try:
            return with_timeout(graceperiod, self.wait)
        except Timeout:
    ...
    def __reader(self, collector, source):
        ...
        """
        while True:
            data = os.read(source.fileno(), 1)#65536)
            self.__lock.acquire()
